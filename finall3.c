    ...
    for (i = 0; i < G.n; i++) {
        for (j = 0; j <G.n; j++) {
            if (G.edges[i][j] == M) {
                ...
                printf("%-11d", G.edges[i][j]);
            }
        }
        printf("\n");
    }
}
void VexsAdd(MGraph *G) {
    printf("\n请输入需要添加的城市信息：");
    G->n++;
    scanf("%s", G->vexs[G->n]);
}
void VexsDel(MGraph *G) {
    G->n--;
}
void VexsChange(MGraph *G) {
    int i, loc;
    str changeCity;
    printf("请输入需要修改的城市信息：");
    scanf("%s", changeCity);
    for (i = 0; i < G->n; i++) {
        if (strcmp(changeCity, G->vexs[i]) == 0) {
            loc = i;
            break;
        }
    }
...
void EdgesAdd(MGraph *G) {
    int i, loc1, loc2, w;
    str str1, str2;
    printf("请输入需要添加的连接信息 （城市1 城市2 权值）");
...
void EdgesDel(MGraph *G) {
    int i, loc1, loc2;
    str str1, str2;
    printf("请输入需要删除的连接信息 （城市1 城市2）");
    scanf("%s %s", str1, str2);
    for (i = 0; i < G->n; i++) {
        if (strcmp(str1, G->vexs[i]) == 0) {
            loc1 = i;
...
void EdgesChange(MGraph *G) {
    int i, loc1, loc2, w;
    str str1, str2;
    printf("请输入需要修改的连接信息 （城市1 城市2 权值）");
    scanf("%s %s %d", str1, str2, &w);
    for (i = 0; i < G->n; i++) {
        if (strcmp(str1, G->vexs[i]) == 0) {
            loc1 = i;
